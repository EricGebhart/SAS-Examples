    ...
    define event options_setup;
        ...
        define event calculate_width;
            ...
            do /if index(value, '%') > 0;
                ...
                putlog "WIDTH:" $width;
                set $img_width compress($width, '%');
                putlog "IMG_WIDTH:" $img_width;
                eval $img_width inputn($img_width, 'BEST20.');
                putlog "IMG_WIDTH:" $img_width;
                /* it gets divided back down again later... */
                eval $img_width ($img_width/100) * 300;
                break;
            done;                
