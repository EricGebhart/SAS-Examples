...
proc template;
    ...
    define event options_setup;
        ...
        define event calculate_width;
            ...
            set $max scan(Tagattr, 2, '-') /if $dash_pos;

            do /if $max & index($max, ' ');
                set $offset scan($max, 2, ' ') ;
                set $max scan($max, 1, ' ') ;
            done;
            ...
            /*------------------------------------------------------------13Feb06-*/
            do /if $offset;
                eval $max inputn($max, 'BEST20.');
                eval $offset inputn($offset, 'BEST20.');
        ...
        define event data;
            ...
            finish:
                ...
                trigger empty_cell_value /if ^$wrote_cell;
...
ods tagsets.slider file='example17.html' style=slider options(doc='help' aural_headers='yes');
